...
## 2026-09-12 - Simplified discovery interface
Removed Genres discovery and its empty-state shortcut. Discovery now has only For You and Based On; taste collections open a matching library search. Genre metadata, matching constraints, the glossary and avoided-genre preferences remain. Refine, advanced experiments, diagnostic exports, source/feedback details and match explanations use collapsed disclosures. Content preferences remain accessible, with the current content section in the summary.
Refreshed typography, warm green palette, spacing, navigation, onboarding and result cards; synopsis precedes actions, with two-column desktop and one-column mobile layouts. Verified local navigation, demo result rendering, keyboard disclosure access and no horizontal overflow at the mobile viewport. Typecheck and production build pass. Recommendation scoring was not changed in this UI update.

## User feedback: preserve genre discovery strengths (2026-09-12)
The user felt the removed Genres mode produced relatively good matches. Keep its retrieval approach as a comparison baseline while improving For You and Based On; do not interpret removal of the tab as rejection of genre-based retrieval. The former mode used a selected genre as the query vector and retrieved provider candidates from that genre, then applied existing ranking and preference/content filters. Compare that simpler, broader genre-first candidate pool against the newer seed-specific subject/subgenre constraints on the same user-reviewed examples. Investigate whether overly narrow constraints or incomplete metadata exclude good matches. Preserve the two-tab UI. This is qualitative user feedback, not a measured benchmark result.
