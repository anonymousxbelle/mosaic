...
const results=data.cases.map(c=>{
 ...
 const baseline=recommend(c.candidates,vector(c.focusTags||c.seed.tags),c.category||'All',[c.seed.id],c.requiredGenre,c.seed);
 ...
 const methods={standard:baseline,provider,providerBlend:hybridRank(baseline,evidence),semanticBlend:hybridRank(baseline,evidence,c.semanticScores||{})};
 return {id:c.id,candidates:c.candidates.length,eligible:baseline.length,
  ...
  methods:Object.fromEntries(Object.entries(methods).map(([method,rows])=>[method,{...evaluateRanking(rows.map(x=>x.id),judgments),top:rows.slice(0,10).map(x=>x.id)}]))};
});
console.log(JSON.stringify({label:data.label||'Unspecified evaluation',note:'Metrics concern the supplied judged pool, not whole-catalog accuracy. Null metrics indicate missing judgments. Ranking is measured before diversity.',results},null,2));
