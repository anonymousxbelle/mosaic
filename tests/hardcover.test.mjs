...
test('Hardcover uses fixed read-only queries and metadata-only discovery fields',async()=>{const old=fetch;const calls=[];globalThis.fetch=async(url,init)=>{calls.push({url,body:JSON.parse(init.body),auth:init.headers.Authorization});return new Response(JSON.stringify({data:{search:{results:{hits:[{document:{id:1,title:'Book',slug:'book',genres:['Fantasy']}}]}}}}));};try{const x=await hardcoverBooks({HARDCOVER_TOKEN:'private-token'},'/discover','','',['fantasy','my-private-list']);assert.equal(x.length,1);assert.equal(calls.length,1);assert.equal(calls[0].auth,'Bearer private-token');assert.ok(calls[0].body.query.includes('genres,description'));assert.equal(JSON.stringify(x).includes('private-token'),false);assert.equal(calls[0].body.query.includes('mutation'),false);}finally{globalThis.fetch=old;}});
